...
static void drawline_dda(const Point *const first, const Point *const last, _Bool framebuffer[HEIGHT][WIDTH]){
	...
	*/
	...
	for (int t=0; t<=steps; ++t) {
		FRAMEBUFFER((int)roundf(x), (int)roundf(y))=1;
		printf("x=%f, y=%f\n", x, y);
		x+=slope_x;
		y+=slope_y;
	}
...
int main(){
	...
	for (int i = 0; i < AMOUNT_OF_DATASET; ++i) {
		...
		drawline_dda(&first, &last, framebuffer);
		printf("Octant %d\n", i);
		display(framebuffer);
