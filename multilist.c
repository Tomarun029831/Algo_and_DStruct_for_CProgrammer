...
void init(){
	for(int i = 0; i < MAX_GAKUSEI; ++i) gakusei[i].glink = NULL;
	for(int i = 0; i < MAX_KAMOKU; ++i) kamoku[i].klink = NULL;
}
...
short get_score(int x, int y){
	for (struct SEISEKI *p = gakusei[x].glink; p != NULL; p=p->glink)
		if(p->kamoku == kamoku+y) return p->seiseki;
	return -1;
}

void print_kamoku(int y){
	puts("kamoku");
	for (struct SEISEKI *p = kamoku[y].klink;p!=NULL;p=p->klink)
